...
{
    public partial class QuotePrint : KryptonForm
    {
        public QuotePrint()
        {
            InitializeComponent();
        ...
        public QuotePrint(string saleno)
        {
            InitializeComponent();
        ...
        public QuotePrint(string saleno, string JobApplied)
        {
            InitializeComponent();
            toolsaleno.Text = saleno;
            JobApply = JobApplied;
        ...
        public QuotePrint(string saleno, bool printID)
        {
            DirectPrint = printID;
        }
        ...
        private void POSPrintRpt_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet Qds = DAL.Quote.GetQuoteReportData(toolsaleno.Text);
                DataTable dt = Qds.Tables[0];

                string cusID = dt.Rows[0]["due"].ToString();
                ...
                vehicleno = dt.Rows[0]["paytype"].ToString();
                string VehicleSQL = @"SELECT [ID],[RegNo],[Make],[Model],[Year],[CustomerID] FROM [dbo].[tblVehicle] WHERE RegNo='" + vehicleno + "'";
                DataTable dtVehicle = DAL.DataAccessManager.GetDataTable(VehicleSQL);
                if (dtVehicle.Rows.Count != 0)
                {
                    VehicleMake = dtVehicle.Rows[0]["Make"].ToString();
                    VehicleModel = dtVehicle.Rows[0]["Model"].ToString();
